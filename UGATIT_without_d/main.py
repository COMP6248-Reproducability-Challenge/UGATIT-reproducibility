...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description=desc) #全局变量
...
def check_args(args):
    # --result_dir
    check_folder(os.path.join(args.result_dir, args.dataset, 'model')) #检查文件夹是否存在，如果不存在则新建。resutl/dataset/model
...
def main():
    ...
    # open session
    gan = UGATIT(args)  # 这样gan这个变量就是一个UGATIT对象，可以使用的函数为tran, test, build_model, save, load
...
if __name__ == '__main__': #python程序入口，习惯
    main()
