...
class UGATIT(object) :
    ...
    def test(self):
        ...
        for n, (real_A, _) in enumerate(self.testA_loader):
            ...
            fakeA = RGB2BGR(tensor2numpy(denorm(fake_A2B[0])))
            realA = RGB2BGR(tensor2numpy(denorm(real_A[0])))
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'A2B_%d.png' % (n + 1)), A2B * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'realA', 'A2B_%d.png' % (n + 1)), realA * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeA', 'A2B_%d.png' % (n + 1)), fakeA * 255.0)
        ...
        for n, (real_B, _) in enumerate(self.testB_loader):
            ...
            fakeB = RGB2BGR(tensor2numpy(denorm(fake_B2A[0])))
            realB = RGB2BGR(tensor2numpy(denorm(real_B[0])))
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B2A_%d.png' % (n + 1)), B2A * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeB', 'B2A_%d.png' % (n + 1)), fakeB * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'realB', 'B2A_%d.png' % (n + 1)), realB * 255.0)
