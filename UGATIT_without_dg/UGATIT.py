...
class UGATIT(object) :
    ...
    def test(self):
        ...
        for n, (real_A, _) in enumerate(self.testA_loader):
            ...
            fake_A = RGB2BGR(tensor2numpy(denorm(fake_A2B[0])))
            real_A = RGB2BGR(tensor2numpy(denorm(real_A[0])))
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'A2B_%d.png' % (n + 1)), A2B * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeA','A2B_%d.png' % (n + 1)), fake_A * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'realA','A2B_%d.png' % (n + 1)), real_A * 255.0)
        for n, (real_B, _) in enumerate(self.testB_loader):
            ...
            fake_B = RGB2BGR(tensor2numpy(denorm(fake_B2A[0])))
            real_B = RGB2BGR(tensor2numpy(denorm(real_B[0])))
            
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'B2A_%d.png' % (n + 1)), B2A * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'fakeB', 'A2B_%d.png' % (n + 1)), fake_B * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'realB', 'A2B_%d.png' % (n + 1)), real_B * 255.0)